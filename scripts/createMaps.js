            ...
            if (buffer) {
                ...
                for (var y = YMinMapSize; y <= YMaxMapSize; y++) {
                    ...
                    for (var x = XMinMapSize; x <= XMaxMapSize; x++) {
                        ...
                        MapData[mapNumber][y][
                            x
                        ].graphics[1] = bufferRcv.readUInt16();

                        if (ByFlags & 2) {
                            MapData[mapNumber][y][
                                x
                            ].graphics[2] = bufferRcv.readUInt16();
                        }

                        if (ByFlags & 4) {
                            MapData[mapNumber][y][
                                x
                            ].graphics[3] = bufferRcv.readUInt16();
                        }

                        if (ByFlags & 8) {
                            MapData[mapNumber][y][
                                x
                            ].graphics[4] = bufferRcv.readUInt16();
                        }

                        if (ByFlags & 16) {
                            MapData[mapNumber][y][
                                x
                            ].trigger = bufferRcv.readUInt16();
                        }
                    }
                }

                var inf = fs.createReadStream(
                    "Maps/Mapa" + mapNumber + ".inf",
                    {
                        end: false
                    }
                );
                            ...
                            for (var y = YMinMapSize; y <= YMaxMapSize; y++) {
                                ...
                                for (
                                    ...
                                    if (ByFlags & 1) {
                                        ...
                                        MapData[mapNumber][y][
                                            x
                                        ].tileExit.map = bufferRcv.readUInt16();
                                        MapData[mapNumber][y][
                                            x
                                        ].tileExit.x = bufferRcv.readUInt16();
                                        MapData[mapNumber][y][
                                            x
                                        ].tileExit.y = bufferRcv.readUInt16();
                                    }

                                    if (ByFlags & 2) {
                                        MapData[mapNumber][y][
                                            x
                                        ].npcIndex = bufferRcv.readUInt16();
                                    }
                                    ...
                                    if (ByFlags & 4) {
                                        ...
                                        MapData[mapNumber][y][
                                            x
                                        ].objInfo.objIndex = bufferRcv.readUInt16();
                                        MapData[mapNumber][y][
                                            x
                                        ].objInfo.amount = bufferRcv.readUInt16();
                                    }
                                }
                            }
